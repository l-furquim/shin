                ...
                <form [formGroup]="form" class="space-y-5">
                  ...
                  @if (saveSuccess()) {
                    <z-alert
                      zType="default"
                      zTitle="Salvo!"
                      zDescription="Detalhes salvos com sucesso."
                    />
                  }

                  <z-button
                    [zDisabled]="form.invalid || isSaving() || !activeVideoId()"
                    [zLoading]="isSaving()"
                    class="w-full"
                    (click)="onSaveDetails()"
                  >
...
export class UploadPageComponent {
  ...
  async onSaveDetails(): Promise<void> {
    ...
    try {
      ...
      this.saveError.set('Não foi possível salvar os detalhes. Tente novamente.');
    } finally {
